...
def generate_cards(bins_data, count):
    ...
    while len(cards) < count:
        ...
        if year == "2026":
            month = str(random.randint(8, 12)).zfill(2) # 8-12
        else:
            month = str(random.randint(1, 12)).zfill(2) # 1-12
...
print(f"✅ {len(CARDS_LIST)} کارتی تەواو جیاواز دروستکران.")
...
async def send_card_message(bot, channel, admin):
    ...
    while True:
        try:
            ...
            elif "Timed out" in err_msg:  # 🔹 گۆڕانکاریی گرنگ بۆ چارەسەری کێشەکە
                print("⚠️ هەڵەی Timed out ڕوویدا. چاوەڕوانی 10 چرکە و دووبارە دەکەین...")
                await asyncio.sleep(10)
                # نەگەڕێتەوە، دەچێتە سەرەتای لۆپەکە و هەمان کارت دووبارە دەنێرێت
            else:
                print(f"❌ هەڵەی تر: {err_msg}")
                return False
...
async def main():
    ...
    print(f"🚀 بۆت دەستی پێکرد بۆ ناردنی {NUMBER_OF_CARDS} کارت...")
    while True:
